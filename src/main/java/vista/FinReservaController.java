 ...
 * @author a
 */
public class FinReservaController implements Initializable {
    ...
    private final coordinador coordinador = new coordinador();
    @FXML
     ...
     */
    //esta clase sirve para cuando un cliente acaba su alquiler y devuelve el vehiculo.
    //el vehiculo se marca en la base de datos como que esta libre(enUso=false)
    @Override
...
//recarga info del tableView
    ...
    @FXML
    private void handleButtonHecho(ActionEvent event) {
        Automobil auto = tableViewCoches.getSelectionModel().getSelectedItem();
        if (new DAOmysql().marcaEnUso(auto, false)) {
            new logica().mostrarDialeg(AlertType.CONFIRMATION, "Hecho", null, "Operacion realizada con exito");
        } else {
            new logica().mostrarDialeg(AlertType.ERROR, "Selecciona un vehiculo", null, "No has seleccionado ningun vehiculo");
        }
        recargaInfo();
    ...
    private void recargaInfo() {
        ObservableList = FXCollections.observableList(new DAOmysql().obtenerTodosCochesNoDisponibles());
    ...
    private void onActionButtonInfo(ActionEvent event) {
        try {
            new logica().mostrarDialeg(Alert.AlertType.INFORMATION, "Informacion", null, new flujosDatos().lee("/info/infoFinReserva.txt"));
        } catch (Exception a) {
